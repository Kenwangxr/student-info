...
<div class="container">
		<div class="row">
			<div class="col-md-12">
			<h5>查看已修课程</h5>
			<p class="alert alert-info">提示：如成绩有误请联系代课教师或管理员</p>
			...
			<table  style="overflow:scroll;" class="table table-striped table-hover animated fadeIn">
				...
				<c:forEach var="ec" items="${requestScope.endCourseList}"
					varStatus="loop">
					<tr>
						<td>${ec.cid }</td>
						<td>${ec.cname }</td>
	...
	<div class="row">
		...
		<div class="col-md-6">
			...
			<ul class="pagination">
				<li><a href="/StudentHandler/endcourse/${sessionScope.sid }/1">首页</a></li>
				<c:if test="${pageInfo.hasPreviousPage }">
					<li><a href="/StudentHandler/endcourse/${sessionScope.sid }/${pageInfo.pageNum-1}"
				...
				<c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
					...
					<c:if test="${page_Num != pageInfo.pageNum }">
						<li><a href="/StudentHandler/endcourse/${sessionScope.sid }/${page_Num }">${page_Num }</a></li>
					</c:if>
				...
				<li><a href="/StudentHandler/endcourse/${sessionScope.sid }/${pageInfo.pages}">末页</a></li>
			</ul>
			</nav>
		</div>
	</div>
