package net.wangxr.xk.handler;

import net.wangxr.xk.pojo.Student;
import net.wangxr.xk.pojo.Teacher;
import net.wangxr.xk.service.CoursePlanService;
import net.wangxr.xk.service.CourseService;
import net.wangxr.xk.service.SelectCourseService;
import net.wangxr.xk.service.StudentService;
import net.wangxr.xk.service.TeacherService;
...
public class AdminHandler {
 ...
 // 添加
 	...
 	public String search(@RequestParam("serc") String serc, @RequestParam("condition") String condition,HttpServletRequest request,
		...
		request.setAttribute("condition", condition);
 		if (serc.equals("all")) {//查询全部
 			...
 			return "admin/queryStudent";
 		} else if (serc.equals("pro")) {//根据专业查询
 			...
 			return "admin/queryStudent";
 		} else if (serc.equals("cla")) {//根据班级查询
	 ...
	 */
 	...
 	public String deleteStudent(@PathVariable(value = "sid") String sid, Model model) {
		if(StringUtils.isEmpty(sid)){
			return "fail";
		}else if (studentService.deleteStudent(sid) != 0) {
	 ...
	 */
 	...
 	public String addTeacher(Teacher teacher, Model model, HttpSession httpSession) {
		if(StringUtils.isEmpty(teacher.getTid())
				|| StringUtils.isEmpty(teacher.getTname())
				|| StringUtils.isEmpty(teacher.getTpassword())){
			return "fail";
		}else if (teacherService.insertTeacher(teacher) != 0) {
	...
	@RequestMapping(value = "/deleteTea/{tid}", method = RequestMethod.GET)
 	public String deleteTeacher(@PathVariable(value = "tid") String tid, Model model) {
		if(StringUtils.isEmpty(tid)){
			return "fail";
		}else if (teacherService.deleteTeacher(tid) != 0) {
 			return "success";
 		} else {
 			System.out.println("fail");
 			return "fail";
